...
function collectRuns(xml: string): IRunInfo[] {
  const runs: IRunInfo[] = [];
  let logicalCursor = 0;
...
export async function extractVariablesFromDocx(fileArrayBuffer: ArrayBuffer): Promise<string[]> {
...
export async function fillDocxTemplate(
  ...
  while ((m = VARIABLE_PATTERN.exec(logicalText)) !== null) {
    const name = m[1].trim();
    matches.push({ start: m.index, end: m.index + m[0].length, value: values[name] ?? '' });
  }
