...
function statusBadge(statut: Template['statutMapping']): JSX.Element {
  ...
  return (
    <div>
      ...
      <p className="subtitle">
        Dépôt et historisation des templates Word (variables au format <code>{'{{Variable}}'}</code>) et correspondance
        avec les colonnes du fichier de conditions commerciales.
      </p>

      {error && <div className="alert error">{error}</div>}

      <div className="panel">
        <button onClick={() => setShowUpload((v) => !v)}>{showUpload ? 'Annuler' : '+ Déposer un template'}</button>
        {showUpload && (
          <UploadTemplateForm
            onDone={async () => {
              setShowUpload(false);
        ...
        <table>
          ...
          <tbody>
            {templates.map((t) => (
              <tr key={t.id}>
                <td>
                  <a href={t.cheminStockage} target="_blank" rel="noreferrer">
                    {t.libelle}
                  </a>
                </td>
...
function UploadTemplateForm({ onDone }: { onDone: () => Promise<void> }): JSX.Element {
      ...
      <div className="form-row">
        <div>
          ...
          <input
            ...
            placeholder="Contrat Beauty — Capillaire"
          />
        </div>
        <div>
...
function MappingPanel({
  ...
  async function downloadBlankMapping(): Promise<void> {
    setBusy(true);
    setError(null);
    try {
      const buffer = await templatesService.buildBlankMappingWorkbook(template.id, conditionsVersionId || undefined);
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `correspondance_${template.libelle}.xlsx`;
      a.click();
  ...
  async function importMapping(): Promise<void> {
    ...
    setInfo(null);
    try {
      const res = await templatesService.importMapping(template.id, file, conditionsVersionId || undefined);
  ...
  return (
    <div className="panel">
      ...
      <div className="form-row">
        ...
        <button className="secondary" type="button" disabled={busy} onClick={downloadBlankMapping}>
