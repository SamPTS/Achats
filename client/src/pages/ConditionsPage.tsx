...
import type { ConditionsVersion } from '../types';
...
export default function ConditionsPage() {
  ...
  async function handleFile(file: File) {
    ...
    setBusy(true);
    try {
      const created = await uploadConditions(file, deposePar);
      const warn =
  ...
  return (
    <div>
      ...
      <p className="subtitle">
        Dépôt et historisation des fichiers Excel de conditions commerciales fournisseurs. Chaque dépôt
        crée une nouvelle version — aucune version n'est jamais écrasée.
      </p>
...
function VersionRow({ version, onChanged }: { version: ConditionsVersion; onChanged: () => Promise<void> }) {
  const [busy, setBusy] = useState(false);
  ...
  async function activate() {
    setBusy(true);
    try {
      await activateConditions(version.id);
      await onChanged();
    } finally {
      setBusy(false);
    }
  ...
  async function saveCol() {
    setBusy(true);
    try {
      await setConditionsCodeColumn(version.id, col);
  ...
  return (
    <tr>
      ...
      <td>
        <a href={downloadConditionsUrl(version.id)}>{version.nomFichier}</a>
      ...
      <td>
        {version.doublonsDetectes > 0 && (
          <span className="badge warn">{version.doublonsDetectes} doublon(s)</span>
        )}
      </td>
      <td>
