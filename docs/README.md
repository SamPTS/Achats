...
conditions commerciales unique sans historique, liaison à un fichier externe (jamais copié), et permissions basées
sur les niveaux d'autorisation SharePoint (Propriétaires / Membres). Les captures d'écran (`images/`) sont des
reproductions fidèles de cette interface (mêmes composants, mêmes styles), avec des données fictives à titre
d'illustration.
...
pas de rôles) — il n'est pas couvert par ces documents.
