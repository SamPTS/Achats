...
// Capture: attrs of <w:t ...> tag, and inner content, tolerant of self-closing empty tags being absent.
...
export async function extractVariablesFromDocx(buffer: Buffer): Promise<string[]> {
 ...
 */
export async function fillDocxTemplate(buffer: Buffer, values: Record<string, string>): Promise<Buffer> {
  const zip = await JSZip.loadAsync(buffer);
  const file = zip.file(DOCUMENT_XML_PATH);
  if (!file) throw new Error("Fichier Word invalide : word/document.xml introuvable.");
  const xml = await file.async('string');
